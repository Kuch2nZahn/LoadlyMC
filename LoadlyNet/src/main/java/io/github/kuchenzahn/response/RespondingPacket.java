...
public class RespondingPacket<T extends LoadlyPacket> {

    private static final Long2ObjectMap<PendingResponse<?>> pendingResponses = new Long2ObjectArrayMap<>();
    private final LoadlyPacket toSend;
    private final long timeout;
    private final Class<T> responseType;
    private final Consumer<T> callback;

    public RespondingPacket(LoadlyPacket toSend, long timeout, Class<T> responseType, Consumer<T> callback) {
        this.toSend = toSend;
    ...
    public RespondingPacket(LoadlyPacket toSend, Class<T> responseType, Consumer<T> callback) {
    ...
    public static void callReceive(LoadlyPacket loadlyPacket) {
        if (!pendingResponses.containsKey(loadlyPacket.getSessionId())) {
            return;
        }
        pendingResponses.get(loadlyPacket.getSessionId()).callResponseReceived(loadlyPacket);
        pendingResponses.remove(loadlyPacket.getSessionId());
